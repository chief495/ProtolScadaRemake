...
namespace ProtolScadaRemake.Controls
{
    public partial class FrameReceptPage : UserControl
    {
        ...
        private IEnumerable<string> GetAliasCandidatesInternal(string tagName)
        {
        ...
        private TVariableTag FindVariableCaseInsensitiveInternal(string tagName)
        {
        ...
        private TCommandTag FindCommandCaseInsensitiveInternal(string tagName)
        {
        ...
        private void UpdateTextBox(TextBox textBox, string tagName, string sectionName)
        {
            ...
            var tag = LookupVariableByNameOrAliasInternal(tagName);
            if (tag != null)
            {
        ...
        private TVariableTag LookupVariableByNameOrAliasInternal(string tagName)
        {
            foreach (var candidate in GetAliasCandidatesInternal(tagName))
            {
            ...
            foreach (var candidate in GetAliasCandidatesInternal(tagName))
            {
                var tag = FindVariableCaseInsensitiveInternal(candidate);
                if (tag != null)
                    return tag;
            }
        ...
        private TCommandTag LookupCommandByNameOrAliasInternal(string tagName)
        {
            foreach (var candidate in GetAliasCandidatesInternal(tagName))
            {
            ...
            foreach (var candidate in GetAliasCandidatesInternal(tagName))
            {
                var command = FindCommandCaseInsensitiveInternal(candidate);
                if (command != null)
                    return command;
            }
        ...
        {
            var tuneTag = LookupVariableByNameOrAliasInternal(tagName);
            if (tuneTag != null)
            {
        ...
        private void SaveSection(string sectionName, Button button)
        {
            ...
            foreach (var tb in textBoxes)
            {
                ...
                var variable = LookupVariableByNameOrAliasInternal(tagName);
                if (variable == null)
                {
                ...
                var command = LookupCommandByNameOrAliasInternal(tagName);
                if (command != null)
                {
        ...
        private void SendCommand(string commandName, string value, string logMessage)
        {
            ...
            var command = LookupCommandByNameOrAliasInternal(commandName);
            if (command != null)
            {
