...
namespace ProtolScadaRemake
{
    public partial class DialogElementPumpUz : Window
    {
            ...
            try
            {
                ...
                if (VariableTag != null)
                {
                    if (VariableTag.ValueReal > 0)
                    {
                        ...
                        SetManualControlsVisibility(Visibility.Visible);
                    }
                    else
                    {
                        RBAuto.IsChecked = true;
                        RBManual.IsChecked = false;
                        SetManualControlsVisibility(Visibility.Hidden);
                    }
                }
                ...
                if (VariableTag != null)
                {
                    if (VariableTag.ValueReal >= 0 && VariableTag.ValueReal <= 100)
                        ManualSpeedNumeric.Value = VariableTag.ValueReal;
                }
        ...
        /// <summary>
        /// Показать/скрыть элементы ручного управления
        /// </summary>
        private void SetManualControlsVisibility(Visibility visibility)
        {
            StartButton.Visibility = visibility;
            StopButton.Visibility = visibility;
            ManualSpeed.Visibility = visibility;
        }
        ...
        private void UpdateVisualStates()
        {
            ...
            UpdateNumericBackground(ManualSpeedNumeric, VarName + "_ManualSpeed");
        ...
        private void ManualSpeedNumeric_ValueChanged(object sender, RoutedPropertyChangedEventArgs<double?> e)
        {
            if (_isInitializing) return;
            if (Global == null || !Global.Access) return;
            if (!ManualSpeedNumeric.Value.HasValue) return;

            TVariableTag ManualSpeedVariable = Global.Variables.GetByName(VarName + "_ManualSpeed");

            if (ManualSpeedVariable != null)
            {
                if (Math.Abs(ManualSpeedVariable.ValueReal - ManualSpeedNumeric.Value.Value) >= 0.001)
                {
                    string value = ManualSpeedNumeric.Value.Value.ToString(System.Globalization.CultureInfo.InvariantCulture);
                    SendCommand("_ManualSpeed", value, $"Скорость в ручном режиме изменена на {value} %.");
                }
            }
        }
